...
当前实现：
  ...
  - resource.zip → <SKILL_DIR>/resource/

用法：
    python setup_resources.py          # 仅下载缺失资源
    python setup_resources.py --force  # 强制重新下载
    python setup_resources.py --resource-url "https://example.com/resource.zip"
    set RESOURCE_ZIP_URL=https://... & python setup_resources.py
    set RESOURCE_ZIP_PATH=C:/path/to/resource.zip & python setup_resources.py
...
# resource.zip 下载源；可通过 --resource-url 或环境变量 RESOURCE_ZIP_URL 指定；本地文件可用 RESOURCE_ZIP_PATH
RESOURCE_ZIP_URL = "https://github.com/neptune46/blobimage/releases/download/release_v0.2/resource.zip"
...
def _check_python_version() -> None:
    """检查 Python 版本，若低于 3.10 则打印警告。"""
    major, minor = sys.version_info[:2]
    ver_str = f"{major}.{minor}.{sys.version_info.micro}"
    if major > 3 or (major == 3 and minor >= 10):
        print(f"[python] ✓ Python {ver_str}（满足 >= 3.10 要求）")
    else:
        print(f"[python] ⚠ 当前 Python 版本：{ver_str}，需要 Python >= 3.10", file=sys.stderr)
...
def main() -> int:
    ...
    ok = True
    do_ffmpeg = not args.resource_only
    do_resource = not args.ffmpeg_only

    steps = sum([do_ffmpeg, do_resource])
    step = 0
